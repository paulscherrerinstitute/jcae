...
/**
 * Integer process variable that executes a shell script if the variable is set to 1 
 */
	...
	protected CAStatus writeValue(DBR dbr, ProcessVariableWriteCallback processvariablewritecallback) throws CAException {
		...
		if(value == 1){
			...
			try {
				...
				ProcessStreamProcessor processor = new ProcessStreamProcessor(process.getInputStream());
				processor.start();
